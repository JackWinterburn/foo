...
export function Dijkstra({grid, startNode, targetNode}: {grid: Node[][], startNode: Node, targetNode: Node}) {
...
const getNeighbors = (node: Node, grid: Node[][]): Node[] => {
  ...
  const calculateHeuristic = (node: Node, targetNode: { x: number, y: number }): number => {
